...
using Nami.Modules.Owner.Common;
...
namespace Nami.Modules.Owner.Services
{
    public static class CSharpCompilationService
    {
        ...
        private static ImmutableArray<string> _usings = new[] {
            ...
            "Nami.Modules", "Nami.Exceptions", "Nami.Attributes"
        }.ToImmutableArray();
        ...
        public static Type? CompileCommand(string code)
        {
            ...
            code = $@"
                ...
                public sealed class DynamicCommands : NamiModule
                {{
                    {m.Groups["code"]}
                }}";
            ...
            moduleType = outerType?.GetNestedTypes().FirstOrDefault(x => x.BaseType == typeof(NamiModule));
