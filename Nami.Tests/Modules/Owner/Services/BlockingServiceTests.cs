...
namespace Nami.Tests.Modules.Owner.Services
{
    [TestFixture]
    public sealed class BlockingServiceTests : INamiServiceTest<BlockingService>
    {
        ...
        {
            this.Service = new BlockingService(TestDbProvider.Database, false);
        }
        ...
        private async Task AssertBlockedAsync(NamiDbContext db, ulong[] bcExpected, ulong[] buExpected, string?[] bcReasons, string?[] buReasons)
        {
            Assert.That(this.Service.BlockedChannels, Is.EquivalentTo(bcExpected));
            Assert.That(this.Service.BlockedUsers, Is.EquivalentTo(buExpected));
            IReadOnlyList<BlockedChannel> bchns = await this.Service.GetBlockedChannelsAsync();
